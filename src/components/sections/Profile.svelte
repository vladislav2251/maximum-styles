            ...
            <form class="grid gap-5 md:w-1/2">
                <div class="flex flex-col md:flex-row gap-5">
                    <div class="flex flex-col gap-3 w-full md:w-1/2">
                        ...
                        <input class="px-8 py-4 border-solid text-base w-full border font-normal border-[var(--color-gray)] rounded-md text-[var(--color-gray)] transition-all duration-300 focus:text-[var(--color-primary-300)] focus:border-[var(--color-primary-300)] outline-none" type="text" id="firstName" name="firstName" required minlength="2" maxlength="30" placeholder="{translation?.main?.profile?.inputs[0]?.placeholder}"/>
                    </div>
                    <div class="flex flex-col gap-3 w-full md:w-1/2">
                        <label for="lastName">{translation?.main?.profile?.inputs[1]?.label}</label>
                        <input class="px-8 py-4 border-solid text-base w-full border font-normal border-[var(--color-gray)] rounded-md text-[var(--color-gray)] transition-all duration-300 focus:text-[var(--color-primary-300)] focus:border-[var(--color-primary-300)] outline-none" type="text" id="lastName" name="lastName" required minlength="2" maxlength="30" placeholder="{translation?.main?.profile?.inputs[1]?.placeholder}"/>
                    </div>
                </div>
