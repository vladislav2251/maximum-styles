    ...
    <div class="flex items-center justify-between gap-4 md:gap-6">
      ...
      <nav class="hidden md:flex justify-center items-center flex-grow">
        {#each menuItems as element}
          <a
            href={element.href}
            class="relative p-2.5 after:content-[''] after:absolute after:left-0 after:bottom-0 after:h-[2px] after:w-0 after:bg-black hover:after:w-full after:transition-all after:duration-300"
          >
            {element.label}
          </a>
        {/each}
      </nav>
      <address class="flex items-center gap-3">
        <a
          aria-label="phone"
          class="flex items-center gap-2 font-medium text-black text-sm max-xl:hidden"
