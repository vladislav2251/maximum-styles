<script>
    import { cartStore, toggleCart, removeProductFromCart } from '$lib/context/cart.js';
    ...
    $: totalSale = items.reduce((acc, item) => acc + item.sale * item.quantity, 0);
        ...
        {#if items.length > 0}
            <div class="flex flex-col gap-5">
                <div class="flex justify-between mb-5">
                    <button on:click={toggleCart} type="button">{translation?.cart?.back}</button>
                ...
                <div class="flex max-md:flex-col gap-5">
                    <div class="max-h-[300px] overflow-x-hidden overflow-scroll w-full">
                        ...
                        <div class="flex items-center max-sm:flex-col max-sm:gap-5 justify-between md:grid md:grid-cols-5">
                            <button class="sm:hidden grid place-content-end w-full" type="button" on:click={() => handleRemove({ id })}>✕</button>
                            ...
                            <div class="flex sm:flex-col gap-2 items-center">
                                ...
                                <p class="text-base text-[var(--color-gray100)] line-through">${sale * quantity}</p>
                            </div>
                            <button class="max-sm:hidden" type="button" on:click={() => handleRemove({ id })}>✕</button>
                        </div>
                        {/each}
                    </div>

                    <div class="grid gap-3 w-full p-3 border border-solid border-[var(--color-gray100)] rounded-md flex-1">
                        ...
                        <div class="flex md:gap-14 justify-between items-center border-t border-solid border-[var(--color-gray200)]">
