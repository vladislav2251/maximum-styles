...
const currentLangInit = (startValue) => {
	...
	if (typeof window !== "undefined") {
		...
		if (!languagesList.some(lang => lang.code === currentLangCode)) { currentLangCode = "de" };

		const currentLang = languagesList.find(language => language.code === currentLangCode);
		store.set(currentLang);

		store.subscribe(value => {
			const currentLang = languagesList.find(language => language.code === value.code);
